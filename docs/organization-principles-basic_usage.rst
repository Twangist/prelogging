...
* :ref:`warnings-consistency-checking`
    .. hlist::
        ...
        * :ref:`logging_configuration-warnings`
        * :ref:`check`
...
############### NOTE NOTE NOTE  logging_configuration.LCDictBasic docstring  NOTE NOTE NOTE
...
of creating a logging config dict into basic steps:
    ...
    4. Add handler specifications with ``add_handler()`` and/or
       ...
       steps (if they weren't, by default `logging_configuration` will issue a warning). Although
...
Every logging config dict built by `logging_configuration` has the five subdictionaries shown.
...
``root_level`` -- the `logging_configuration` default is the `logging` default.
...
:ref:`diagram of classes <logging_configuration-all-classes-except-ABC>`) Each instance saves
...
A little example:
    ...
    >>> import logging
    >>> lcd = logging_configuration.LCDict(attach_handlers_to_root=True)
...
at their respective `logging_configuration` default loglevels ``'WARNING'`` and ``'NOTSET'``;
...
`logging_configuration` warnings and consistency checking
-----------------------------------------------------------

Added benefit provided by `logging_configuration` that you don't enjoy by handing a big
hand-coded dict to `logging.config.dictConfig()``.

`logging_configuration` detects certain dubious practices, automatically corrects some of them
...
`logging_configuration` warnings
++++++++++++++++++++++++++
...
that indicate the different kinds of anomalies that `logging_configuration` checks for, corrects
...
`logging_configuration` detects and eliminates duplicates in lists of handlers or filters
that are to be attached to higher-level entities. If ``REATTACH`` is turned on
in ``warnings``, `logging_configuration` will report the duplicate (by printing a warning message
