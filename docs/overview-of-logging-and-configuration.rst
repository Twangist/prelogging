...
before or in tandem with the `logging_config` documentation. Nevertheless, it will be
...
YAML may be more readable than dictionary specifications, but `logging_config` offers
...
<lead-in to next chapter -- `logging_config` provides a middle ground / hybrid / best of both worlds;
 blah blah>
...
contains a wealth of techniques, several of which exceed the scope of `logging_config` because
...
). A few of the examples contained in the `logging_config` distribution are examples from
the Cookbook and HOWTO, reworked to use `logging_config`.
...
Happily, `logging_config` does, in a couple of ways.
