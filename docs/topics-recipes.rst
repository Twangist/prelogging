...
* :ref:`using-prologging-with-django`
    .. hlist::
        :columns: 3
...
.. _using-prologging-with-django:

Using `prologging` with `Django`
------------------------------------
...
.. todo::
    (2) Illustrate how to use `prologging` with Django. In ``settings.py``:
...
.. topic:: Two solutions
    ...
    In the approach provided natively by `prologging`, serialization occurs at the
...
(MP blather)
Provided natively by `prologging`, only option under Py2.
...
Using `prologging` in libraries: using a null handler
-----------------------------------------------------------------
...
illustrate how to use `prologging` in both a library and a program that uses it,
...
``disable_existing_loggers`` to ``False``. This is actually `prologging`\'s default —
one of the few instances where `prologging` changes the default used by `logging`
...
``library`` use of `prologging` and `logging`
++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++

The package contains just two modules: ``__init__.py`` and ``module.py``.

``__init__.py`` configures logging with `prologging`, adding a null handler and
...
``use_library.py`` use of `prologging` and `logging`
++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
and a simple ``prologging`` function::
...
`prologging` supports a majority of the `logging` handlers, but not all.
...
Nevertheless, all can be configured using `prologging`.
